...
class Mob_01_left(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if 206 > self.rect.centerx > 0:
            self.speedx = 3.5
            self.rect.bottom = -0.0123*((self.rect.centerx)**(1.66)) + self.starting_pos[1]
        if 551 >= self.rect.centerx > 206 and self.finished_sequence == 0:
            self.rect.bottom = -150*math.sin(2*math.pi*(self.rect.centerx - 303)/700) + (self.starting_pos[1]-200)
            # print(self.rect.centerx, self.rect.bottom)
            if 551 <= self.rect.centerx:
        ...
        if self.rect.centerx >= self.last_centerx and self.rect.bottom >= self.last_bottom:
            ...
            self.rect.centerx = -0.0015*(self.rect.bottom - 370)**2 + self.starting_pos[1] + 277
